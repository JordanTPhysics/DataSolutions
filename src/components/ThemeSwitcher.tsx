...
function ThemeSwitcher() {
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      setTranslateX((containerWidth / 2.2)); // Adjust this to center the highlight as desired
    }
  }, [containerRef]);

  return (
    <div
      className='theme-switcher text-info border-4 border-gray-900 rounded-full m-auto relative'
      ref={containerRef}
    >
      <div
        className={`absolute w-[50%] h-full bg-gray-900/50 border-2 border-gray-900 rounded-full transition-all duration-300 ease-in-out m-auto z-10`}
        style={{
      ...
      <button
        ...
        className={`dark-mode-switch border-none cursor-pointer 
          text-border z-20 m-0 `}
        onClick={setDarkTheme}
      >
        <FaRegMoon color='navy' size={20} />
      </button>
      <button
        type='button'
        className={`light-mode-switch cursor-pointer 
          text-border border-none z-20 m-0`}
